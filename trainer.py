...
from dataset import Dataset, get_negative_samples
...
class Trainer:
    ...
    def train(self) -> Tuple[int, Dict[str, Any], Dict[str, Any]]:
        best_ndcg = 0
        best_hit_rate = 0
        best_ndcg_epoch = 0
        best_hit_epoch = 0
    ...
    def evaluate(
        self,
        mode: str = "valid",
        model: Optional[SASRec | SSMRec] = None,
    ) -> Tuple[float, float]:
        if mode == "valid":
        ...
        if not isinstance(ndcg, float):
            try:
                ...
                raise TypeError(f"{e} ndcg should be float and couldnt convert")
        return ndcg, hit
